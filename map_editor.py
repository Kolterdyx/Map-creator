...
def draw_colored(octv, scale, sea, width, height, lac, pos, seed, noise):
    ...
    for x in range(width):
        ...
        for y in range(height):
            if noise == 2:
                ...
                n = create_noise(octv, x, y, scale, lac, 0, 100, ngen)
...
def sw():
    ...
    class Screen:
        def create(self):
            ...
            self.opensimplex_option.check_color = 0
            ...
            self.perlin_noise_option.check_color = 0
            ...
            self.simplex_option.check_color = 0
...
def editor():
    ...
    class Editor:
        def __init__(self):
            ...
            self.x_offset_slider = Slider(self, max=SETTINGS["wdh"])
            self.x_offset_slider.move(20, 280)
            self.x_offset_slider.set_length(300)
            self.x_offset_slider.set_font_size(15)
            self.x_offset_slider.set_width(15)
            self.x_offset_slider.set_label("X Padding: 0")
            self.x_offset_slider.set_mark(SETTINGS["wdh"]/2)

            self.y_offset_slider = Slider(self, max=SETTINGS["hgt"])
            self.y_offset_slider.move(20, 320)
            self.y_offset_slider.set_length(300)
            self.y_offset_slider.set_font_size(15)
            self.y_offset_slider.set_width(15)
            self.y_offset_slider.set_label("Y Padding: 0")
            self.y_offset_slider.set_mark(SETTINGS["hgt"]/2)
            ...
            self.widgets = [
                ...
                self.pos_entry,
                self.lac_slider,
                self.x_offset_slider,
                self.y_offset_slider,
                self.seed_entry,
                self.clear_button,
        ...
        def draw(self):
            ...
            for x in range(10):
                for y in range(10):
                    width = int(SETTINGS["wdh"]/10)
                    height = int(SETTINGS["hgt"]/10)
        ...
        def reset_settings(self):
            ...
            self.height_slider.set_mark(50)
            self.lac_slider.set_mark(10)
            self.x_offset_slider.set_mark(300)
            self.y_offset_slider.set_mark(300)
            self.pos_entry.text = '0,0'
            self.seed_entry.text = '0'
        ...
        def update(self):
            ...
            if generation_finished.value:
                ...
                self.map = pg.Surface((width*10, height*10))
                self.map.fill((100, 100, 100))
            ...
            temp_map = pg.transform.scale(self.map, (int(SETTINGS["mwd"]*self.zoom), int(SETTINGS["mhg"]*self.zoom)))
            temp_map_rect = temp_map.get_rect()
            temp_map_rect.center = (300+SETTINGS["xof"]*self.zoom,
                                    300+SETTINGS["yof"]*self.zoom)
            self.map_surface.blit(temp_map, temp_map_rect)
            ...
            self.x_offset_slider.set_label("X Offset: "+str(int(self.x_offset_slider.mark-300)))
            SETTINGS["xof"] = self.x_offset_slider.mark - 300
            self.y_offset_slider.set_label("Y Offset: "+str(int(self.y_offset_slider.mark-300)))
            SETTINGS["yof"] = self.y_offset_slider.mark - 300
